...
把**原版 DeepSeek Harness** 接入 [AI Store](https://aistore.best)：
设备授权登录 + 一个网关 provider，**20 个模型**走同一入口——任何客户端都不需要
你自己的上游 API Key。

> **坦白说明：** AI Store 是 AgentsDance AI 的商业托管服务。整套平台可免费自部署——
...
它会打开浏览器做设备授权（RFC 8628 风格，与 AI Store 桌面端同一条流程），拉取
...
重启 DeepSeek Harness，在模型列表的 **AI Store** 组里选模型即可（例如带 1M
...
- Token 存于 `$DSH_HOME/dsh-cloud-auth.json`（权限 600），以 `DSH_CLOUD_TOKEN`
  ...
  （bash 工具、MCP 服务器）中剥除。
- 可随时在 AI Store 控制台吊销设备，吊销即杀死 token。
...
本连接器采用 Apache-2.0。AI Store 平台本身的许可见
