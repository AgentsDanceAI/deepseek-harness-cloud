#!/bin/bash
# DSH Cloud tunnel — client side, runs ON the prod host (agentsdance, <PROD_HOST_IP>) as root.
# Installs:
...
# and pins <node>'s host key for port <TUNNEL_PORT> in /root/.ssh/known_hosts (must equal the port-22 key).
# Uninstall: systemctl disable --now dsh-tunnel; rm -f the two files above.
set -euo pipefail
R=<NODE_PUBLIC_IP>; P=<TUNNEL_PORT>
...
ip link set "$DEV" up
ip route replace 10.42.0.0/16 dev "$DEV"   # k3s pod CIDR on <node>
ip route replace 10.43.0.0/16 dev "$DEV"   # k3s service CIDR on <node>
...
[Unit]
Description=DSH Cloud L3 tunnel to <node> (k3s node) over ssh :<TUNNEL_PORT>
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/usr/bin/ssh -o BatchMode=yes -o ExitOnForwardFailure=yes -o ServerAliveInterval=15 -o ServerAliveCountMax=3 -o ConnectTimeout=15 -o StrictHostKeyChecking=yes -o UserKnownHostsFile=/root/.ssh/known_hosts -o PermitLocalCommand=yes -o LocalCommand="/usr/local/sbin/dsh-tunnel-local-up %%T" -o Tunnel=point-to-point -w any:0 -p <TUNNEL_PORT> -i /root/.ssh/id_ed25519 root@<NODE_PUBLIC_IP> dsh-tunnel
Restart=always
RestartSec=5s
...
[ -n "$K50" ] || { echo "no host key on $R:$P — is sshd-dsh-tunnel running on <node>?" >&2; exit 3; }
