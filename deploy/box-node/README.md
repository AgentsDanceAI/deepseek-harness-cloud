...
## 定位: 只做备用节点, 不做长期承载 (2026-09-06 老板裁定)

讨论过把 Box 当**日常**的承载形态 —— 一人一台 Box、或者一人一产品一台 —— 结论是**不做**,
判据只有一条: **用户点开工作台要等 79 秒。**

实测 (停机 → 开机 → 公网 URL 真的返回 200):
...
⚠️ 顺带一个坑: 状态 17 秒就报 `ready`, 页面还要再等一分钟 —— **别拿 API 的 state 当就绪
信号**, 那会把人放进一个还没起来的盒子 (和 Dify/Coze 就绪探针踩过的是同一个形状)。

79 秒是 VM 开机的硬底, 压不下去: 不停机就要按 24 小时计费; 闲置保温只救得了同一时段的
第二次进入, 救不了每天第一次点开。所以这条路对**日常**形态直接出局, 不用再算成本。

（成本本身反而不是问题, 记在这里免得有人重新算: Box 按整台按秒计, 停机免费, 规格加权
small 0.5× / default 1× / large 2× / xlarge 5.56×。「一人一台」在产品多而小时更省,
「一人一产品一台」在有一个特别大的时更省 —— 我们目录里 Coze 要 16G 而其余全在 1–4G,
所以真要做也该是「一人一台 + Coze 单开」。这些都不重要了, 因为卡在 79 秒上。）

**这个结论不影响备用节点。** 备用节点是「一台 Box 当一个 k3s 节点」: 79 秒只在**激活那
一次**付 (加上 k3s 起来, 总共两分钟出头的停机), 之后所有用户开工作台都是节点上起 Pod,
还是 2 秒。

另外两件实测结论, 与上面的取舍无关但值得留着:
- **docker 的镜像/容器/命名卷跨停机存活**, 尽管它们在 `/var/lib/docker`(快照文档没列这个
  目录)。ascii.dev 对它做了特殊处理。所以盒子里的镜像是**一辈子拉一次**, 不是每次回来拉。
- `POST /boxes/{id}/host` 给的公网 URL **跨停机稳定**, 开机后原样可用。
