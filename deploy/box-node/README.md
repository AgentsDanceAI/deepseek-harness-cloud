...
> ## ⚠️ 2026-09-08: 这条线作为**节点后备**已被取代, 但没删
>
> 取代它的是 `deploy/k8s-node/join-as-agent.sh` —— 控制面搬到应用机之后, "248 没了
> 怎么办"的答案变成"随便拿一台机器跑一次 join 脚本", 不需要提前备一台等价节点,
> 也不需要切 `.env`。见 `docs/design/cluster-topology.md`。
>
> **留着是因为里面这些还有用**:
> - `box.sh` 的**归属清单 + rm 拦截**。2026-09-08 我用
>   `for b in $(box.sh ls | awk '{print $1}')` 批量删除, 把**整个共享账号**上的盒子
>   全删了, 连带毁掉口袋专家两位用户的云电脑 (删盒子会一起删它的快照, 不可恢复)。
>   **共享账号上的批量删除必须按归属过滤; 例行清理用 `stop`, 永远不要 `delete`。**
> - Box 的几个隐形约束: `/etc/wireguard` 是 ascii.dev 自己的目录, 每次开机重置
>   (我们的配置和私钥被静默清掉, 而 systemd 仍显示 enabled) → 要放 `/opt/dsh-wg`;
>   快照不含 `/var`, k3s 的解压目录会剩一个空壳 → 要 ExecStartPre 自愈。
> - 计费要带组织: API key 默认走个人钱包(试用额度), 症状和"订阅没生效"一模一样 →
>   `X-Box-Org`。
>
> **Box 订阅本身还在用** (口袋专家那条线共用同一个账号), 别退订。
...
## 冷启动实测: 20 秒 (不是 79 秒 —— 那个数是错的)

**这两个数都对, 但量的不是一回事, 别混用:**

| 场景 | 停机 → 服务能应答 |
|---|---|
| **干净盒子** (只有 docker, 服务已预装) | **7 / 19 / 24 / 18 秒 (四轮, 取 ~20 秒)** |
| 备用节点那台 (开机要拉起 k3s + gVisor + 整个集群) | 79 秒 |
| 对照: 节点上起一个 Pod | 2 秒 |

2026-09-06 我先拿备用节点那台量出 79 秒, 并据此说「一人一台 Box」的形态不可行 ——
**那个判断的依据是错的**。那台盒子开机时要把一整个 k3s 集群拉起来, 拿它代表一台
个人盒子不公平。换一台干净的重测 (镜像已在盒内、容器 `restart=always` 已建好),
四轮是 7/19/24/18 秒。

量法上有个必须分开的东西: **盒内 `localhost` 和公网路由是同时通的** (四轮都同秒),
所以那 20 秒全在「VM 开机 + 服务起来」, ascii.dev 的边缘路由不额外收时间。
`/boxes/{id}/host` 的 URL 跨停机稳定, 开机后原样可用。

⚠️ 但**别拿 API 的 `state` 当就绪信号**: 状态 7–9 秒就报 `ready`, 服务还要再等
10–15 秒。放人进去会看到一个还没起来的盒子 (和 Dify/Coze 就绪探针踩过的同一个形状)。

### 那「一人一台」到底行不行

冷启动这一条不再是拦路虎 (20 秒, 而且 Dify/Coze 自己启动就要 30 秒到两分钟, 盒子那
20 秒基本被盖住)。剩下三条是真的, 但都是**成本/运维**问题, 不是体验问题:

1. **内存要按最大的那个配, 所有人一起付。** 盒子得装得下用户可能开的东西; Coze 要
   16G 而 `default` 只有 8G → 想让 Coze 能开, 每人的盒子都得是 `large` (2× 计费,
   要 $100 档)。只用 512MB 小产品的用户也按 16G 付。节点模型下他就是 512MB。
2. **版本漂移。** 我们镜像换得勤 (`codecli-local:...-r10`、`agentui:0.2.4`)。节点模型
   改个 ref, 下一个 Pod 就是新的; 每人一台之后, 底片更新了而**已存在的盒子还是旧的**,
   每台都要单独升级且要停机。ascii.dev 有 `POST /environments/{name}/upgrade`, 但没验过它做什么。
3. **磁盘 N 份。** 自建 12 个镜像 5.55 GB 压缩 (解开 12–14 GB), 加 Dify/Coze 的公共
   中间件。`small` 总盘 40G、`default` 剩 49G —— 装得下, 但每个用户各存一份。

**现状: 备用节点这条线按「一台 Box 当一个 k3s 节点」做完并验过, 不受上面讨论影响**
(激活时付一次 79 秒, 之后所有人开工作台还是节点上起 Pod, 2 秒)。「一人一台」要不要做
是独立的产品决定, 上面三条是它真正的账。
