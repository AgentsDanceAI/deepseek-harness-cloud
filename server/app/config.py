...
CLI_WORKSPACE_IMAGE_REF = _env("CLI_WORKSPACE_IMAGE_REF", "ghcr.io/agentsdancepro/workspace-cli:0.77.0-r2")
...
#: —— 但 09-12 实测发现真正的天花板根本不在这里, 于是这个数是 **2**:
#: GPU 侧 avatar_server 的 MAX_CALLS (env AVATAR_MAX_CALLS) 默认 3, 那是**两条产品线
#: 共用**的总会话数; 而**一间直播只要在播就一直占着一路**, 哪怕整轮都在回放缓存、
#: 一帧都不生成 (播出端 _session 连上 ws 就不放)。所以:
#:   三间直播在播 = 三路占满 = 数字人伴聊的真人用户一个位置都排不上
#:   (实测: 开着三间去点 /avatar 的「开始通话」, 页面写「排队中，前面还有 1 位…」,
#:    排 AVATAR_QUEUE_MAX_WAIT=180 秒之后回 busy)
#: 第四间还会陷入死循环: 它排在队里, 上游不回 ping, 播出端 40 秒 keepalive 超时 →
#: 重连 → 重新排队, 日志里是"上游会话中断"刷屏, 而房间永远出不了流。
#: 2 = 3 路里留 1 路给真人。**要开更多间就得先抬 AVATAR_MAX_CALLS** (在 GPU 节点的
#: deploy/gpu-node/compose.gpu.override.yml, 改完要重建 avatar 容器), 抬到
#: "直播间数 + 给真人留的路数"; 只抬这里不抬那里, 第 3 间起必然起不来。
#: 根治是让在播但不生成的房间别占着一路 —— 命中缓存的整轮本来就不需要上游。
#: 超了不排队, 直接拦, 让管理员先关一间。
LIVE_MAX_CONCURRENT = _env_int("LIVE_MAX_CONCURRENT", 2)
