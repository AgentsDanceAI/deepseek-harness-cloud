...
#: 09-12 抬到 5 (= 配好的全部房间), 实测依据: 三间同播、其中一间正在录第一轮时, 三间
#: 的发布速率都是 0.99~1.00x, GPU 采样 0%, 播出容器 20% 的单核, 16 核机器负载 0.73。
#: **残留风险不是"在播几间", 是"同时录第一轮几间"** —— 只有那一步抢卡, 而这道闸数的
#: 是在播数, 数不出来。所以开新间、以及改完话术(改话术让缓存键全部失效, 等于重录一轮),
#: 都请一间一间来, 等它缓存满了再动下一间。
#: 超了不排队, 直接拦, 让管理员先关一间。
