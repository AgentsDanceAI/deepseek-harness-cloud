...
AGENTS_TEAM_IMAGE_REF = _env("AGENTS_TEAM_IMAGE_REF", "ghcr.io/agentsdancepro/agents-team:0.7.13")
...
CLI_WORKSPACE_IMAGE_REF = _env("CLI_WORKSPACE_IMAGE_REF", "ghcr.io/agentsdancepro/workspace-cli:0.77.0-r8")
# OpenManus 那格的 pi-web-ui 版: 同一个外壳叠在 agent-frameworks 上 (Dockerfile.openmanus),
# 同一个开关 (USE_CLI_WORKSPACE) 管三格 —— 回滚只动一个变量。标签跟外壳同步走。
OPENMANUS_CLI_IMAGE_REF = _env(
...
# ── AI 智慧搜索 / AI 智慧推荐 (2026-09-17, 自口袋专家迁入) ─────────────
# 两格共用口袋专家那台后端的**数据面** (索引、召回、推荐)。今天它与 INFERENCE_URL
# 是同一台机器, 但语义不同 (一个是推理面一个是数据面), 所以分开配 —— 将来数据面
# 搬走时只改这一处。留空 = 这两格不上线。
PE_API_URL = _env("PE_API_URL", "")
# ⛔ 只在服务端用。⚠️ 它是**一把租户密钥** —— 现阶段所有访客共用同一个上游租户的
# 索引与账单, 所以这两格都还 unlisted。要开给所有人得先做"访客 → 上游租户"的映射。
PE_API_KEY = _env("PE_API_KEY", "")
...
# 直播间清单 (逗号分隔的 id, 顺序即页面顺序)。**只有这个名单里的 id 存在** ——
# 房间名会被直接拼进上游 URL, 不钉死就是任人拿我们当探测器去摸 GPU 节点。
# 每间的名称/形象/音色/话术都存在上游, 由管理员在控制台里配, 所以这里只要 id。
LIVE_ROOMS = _env("LIVE_ROOMS", "official")
# 兼容旧配置: 单间时代的 LIVE_ROOM 仍然认, 且它是列表里的第一个 (默认间)。
LIVE_ROOM = _env("LIVE_ROOM", "official")
#: 最多几间同时在播。**这是算力上限, 不是产品策略**, 老板 2026-09-09 定的 3。
#: 09-12 起话术按句缓存 (GPU 侧 live_server "句子缓存"): 循环回放不占卡, 抢卡的只剩
#: 每间**第一轮**录制与回答评论那几句 —— 所以这个数现在远没有 09-09 那么紧
#: (那时一路实时流就占掉那张卡串行吞吐的三分之一)。
#: 09-12 还拆掉过另一道墙: 播出端原来一开播就攥着 avatar 的 ws 不放, 而 avatar 的会话数
#: (MAX_CALLS=3) 是两条产品线共用的 —— 三间在播就把数字人伴聊的真人挤到「排队中」(实测),
#: 第四间自己也永远起不来。现在播出端只在要生成时才连 (第一轮录制 / 回评论), 闲置 20 秒
#: 就放掉 (live_server._Upstream): 整轮回放的房间不占 avatar 任何名额。实测两间在播时
#: 伴聊 1.7 秒接通、全程无 queued; 一条弹幕回复占一路约 40 秒。
#: 所以 5 = 配好的全部房间。真正会抢卡的只剩"同时在录第一轮 / 同时在回评论"的路数, 而这道
#: 闸数的是在播数, 数不出来 —— 开新间、以及改完话术 (缓存键全失效 = 重录一轮), 请一间一间
#: 来, 等它缓存满了再动下一间。超了不排队, 直接拦, 让管理员先关一间。
#: 09-12 再抬到 17 = 十二间百炼音色的新房间 + 原五间。回放不占任何名额 (上游按需连接),
#: 这个数现在只挡"同时录第一轮"的并发 —— 开新间仍然一间一间来。
LIVE_MAX_CONCURRENT = _env_int("LIVE_MAX_CONCURRENT", 17)
