...
def _private_key():
    try:
        ...
        raise HTTPException(500, "wechat_key_invalid")
...
def _auth_header(method: str, path: str, body: str) -> str:
    ...
    sig = base64.b64encode(
        _private_key().sign(msg.encode(), padding.PKCS1v15(), hashes.SHA256())).decode()
    return (f'WECHATPAY2-SHA256-RSA2048 mchid="{config.WECHAT_PAY_MCHID}",nonce_str="{nonce}",'
...
def create_native(order: dict) -> str:
    ...
    body = json.dumps({
        "appid": config.WECHAT_PAY_APPID, "mchid": config.WECHAT_PAY_MCHID,
        "description": order["description"], "out_trade_no": order["order_id"],
        "notify_url": f"{config.PUBLIC_BASE}/api/pay/webhook/wechat",
        "amount": {"total": order["amount_cents"], "currency": "CNY"},
    }, ensure_ascii=False, separators=(",", ":"))
    r = httpx.post(API_BASE + path, content=body, timeout=30,
                   headers={"Authorization": _auth_header("POST", path, body),
...
def query_order(order_id: str) -> dict:
    ...
    r = httpx.get(API_BASE + path, timeout=30,
...
def decrypt_resource(resource: dict) -> dict:
    ...
    plain = aes.decrypt(resource["nonce"].encode(),
...
def process_webhook(body: dict) -> dict | None:
    ...
    try:
        ...
        raise HTTPException(400, "bad_ciphertext")
