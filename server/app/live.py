...
LIVE_PRESETS = [
    ...
    # ── 09-12 按百炼音色成批开的十二套 (老板: "每个音色生成形象和直播间") ──────────
    # 形象 = 同名 png (gpt-image-1.5 按现有五张的风格生成, YuNet 定脸框), 音色 = 百炼
    # qwen3-tts-flash 的系统音色 id (avatar 侧 BAILIAN_VOICE_IDS 认得, 会话里直接指定)。
    # 名字沿用百炼给这把嗓子起的中文名 —— 嗓子就是她的身份, 别再起一个。
    {
        "id": "serena",
        "person": "serena",
        "voice": "Serena",
        "name": "苏瑶",
        "trait": "温柔",
        "persona": "你叫苏瑶。说话温柔舒缓，像晚上讲睡前故事，句子不长，让人放松。",
    },
    {
        "id": "momo",
        "person": "momo",
        ...
        "name": "茉兔",
        "trait": "俏皮",
        "persona": "你叫茉兔。说话俏皮爱撒娇，偶尔搞怪逗人开心，语气词多但不油腻。",
    },
    {
        "id": "vivian",
        "person": "vivian",
        ...
        "name": "十三",
        "trait": "飒",
        "persona": "你叫十三。说话拽拽的、直来直去，有点小暴躁但其实很热心，不绕弯子。",
    },
    {
        "id": "moon",
        "person": "moon",
        ...
        "name": "月白",
        "trait": "帅气",
        "persona": "你叫月白。说话率性帅气、干净利落，懂穿搭，给建议一针见血。",
    },
    {
        "id": "maia",
        "person": "maia",
        ...
        "name": "四月",
        "trait": "知性",
        "persona": "你叫四月。说话知性温柔，讲书像跟朋友聊天，会引一两句原文但不掉书袋。",
    },
    {
        "id": "kai",
        "person": "kai",
        ...
        "name": "阿凯",
        "trait": "治愈",
        "persona": "你叫阿凯。说话低沉温和、节奏很慢，像在做冥想引导，让人安静下来。",
    },
    {
        "id": "katerina",
        "person": "katerina",
        ...
        "name": "卡捷琳娜",
        "trait": "御姐",
        "persona": "你叫卡捷琳娜。说话干练有气场，职场经验丰富，给意见直接、有分寸。",
    },
    {
        "id": "sage",
        "person": "sage",
        ...
        "name": "沧明子",
        "trait": "睿智",
        "persona": "你叫沧明子。说话沉稳睿智、不疾不徐，讲国学像老先生讲故事，偶尔引经据典。",
    },
    {
        "id": "neil",
        "person": "neil",
        ...
        "name": "阿闻",
        "trait": "播音",
        "persona": "你叫阿闻。说话字正腔圆、语调平直，像新闻主播，客观简洁不带情绪。",
    },
    {
        "id": "bellona",
        "person": "bellona",
        ...
        "name": "燕铮莺",
        "trait": "热血",
        "persona": "你叫燕铮莺。说话洪亮有劲、吐字清晰，像健身教练带课，鼓劲但不吼人。",
    },
    {
        "id": "vincent",
        "person": "vincent",
        ...
        "name": "田叔",
        "trait": "江湖",
        "persona": "你叫田叔。说话沙哑沉厚、江湖气足，讲故事像说书，有起伏有停顿。",
    },
    {
        "id": "nini",
        "person": "nini",
        ...
        "name": "邻家妹妹",
        "trait": "软糯",
        "persona": "你叫邻家妹妹。说话软软糯糯、亲切自然，像隔壁妹妹跟你聊校园日常。",
    },
]
...
async def status(room: str = ""):
    ...
    _sample_rate(d, room)  # 顺手算产出速率, 掉出实时会记一条
    return JSONResponse(
        {
            "enabled": True,
            ...
            # 她已经很久没真的说话了(上游挂了/一直起不来), 画面还在走但那是空镜。
            # **必须告诉观众**: 时间轴不断是为了不给硬黑屏, 不是为了让"直播中"这三个
            # 字继续挂着 —— 一个一动不动的人配着"直播中", 比一句诚实的提示难看得多。
...
async def get_room(room: str = "", user: dict = Depends(resolve_user)):
    ...
    # 实测产出速率, **诊断用, 不画在界面上**。低于 1.0 就是生产比消费慢, 缓冲开多大
    # 都会被抽干。这一版本来在控制台上显示过一行, 创始人 2026-09-10 撤掉了 —— 控制台
    # 是给人看的产品界面, 工程内部指标摆上去只会让人问"这是什么"。要看就查这个接口
    # 或 live_incidents 表(产出侧的 slow/recovered 事件)。
    d["rate"] = _rate_now(room)
...
async def say(body: dict, user: dict = Depends(resolve_user)):
    ...
    **插播是排在当前句之后播的, 不打断当前句** —— 打断会把正在生成的那句撕成半截,
    ...
    if mode == "chat":
        ...
        try:
            cfg = await _gpu("GET", f"/rooms/{room}/status", room)
...
def _rate_state(room: str = "") -> dict:
    """某一间的采样窗。按房间分 —— 全局一份的话, 甲间换场把乙间的采样清掉, 或者两间的
    edge 交错着进同一个窗, 算出来的速率是两间的混合, 谁都不对。"""
    return _RATES.setdefault(room or config.LIVE_ROOM, {"pts": [], "slow": False})


#: 默认那间的采样窗 —— 与 _rate_state() 是同一个 dict; 单间时代的测试直接摸它。
_RATE: dict = _rate_state()
...
def _rate_now(room: str = "") -> float:
    """当前窗口内的实测产出速率; 采样不够就回 0 (界面上显示成"测量中")。"""
    pts = _rate_state(room)["pts"]
    if len(pts) < 2:
        return 0.0
...
def _sample_rate(d: dict, room: str = "") -> None:
    ...
    try:
        ...
        if not R["slow"] and rate < _RATE_BAD:
            R["slow"] = True
            _record("slow", "server", room=room, secs=span, detail=f"产出 {rate:.3f}x")
            log.warning("直播产出掉出实时: %.3fx (%.0f 秒窗)", rate, span)
        elif R["slow"] and rate >= _RATE_OK:
            R["slow"] = False
            _record("recovered", "server", room=room, secs=span, detail=f"产出 {rate:.3f}x")
...
async def captions(room: str = ""):
    ...
    _sample_rate(st, room)  # 字幕是三秒一问的, 采样主要靠这里
    lines = [
        {
...
async def incidents(hours: float = 6.0, limit: int = 200, room: str = "", user: dict = Depends(resolve_user)):
    ...
    rows = db.query(
        ...
        (room, since, max(1, min(int(limit or 200), 1000))),
    )
    items = [
        {
            "kind": r["kind"],
