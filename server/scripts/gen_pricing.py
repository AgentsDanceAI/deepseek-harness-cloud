...
# 每工作台每小时 = 算力 ¥0.1414 (1vCPU/2GiB 经济型, 由实测的 0.5/1 线性推)
# + 公网 IP 保有费 ¥0.040 = **¥0.1814**。那 4 分钱不在 ECI 的价目里, 但每个实例
# 都自动创建一个 EIP —— 漏掉它会让下面的平衡点虚高约 28%。
# 在**积分也用满**的前提下, 各档的盈亏平衡机时:
#     Plus 月付 212h / 首月 139h / 年付 102h
#     Pro       511h 起
#     Max      1023h 起
# 原来 Plus 含 180h —— 月付尚有余量, 但首月与年付的含量已经**越过平衡点**,
# 重度用户是净亏。Pro/Max 余量以千小时计, 不受影响。
# 所以只降 Plus: 180h -> 100h (每天 3.3 小时, 对 $10 档合理)。
# ⚠️ 算上 EIP 之后, Plus 年付的余量只剩 **2 小时** (平衡点 102h vs 含量 100h),
# 满用毛利约 1%。三者同时满 (年付 + 积分用满 + 机时用满) 才会发生, 作为拉新档
# 位可接受; 但第一张账单校准后若单价更高, 这一档要重新看。
TIERS = [
