...
# 每工作台每小时 = 算力 ¥0.0707 (0.5vCPU/1GiB 经济型, 创建页实测)
# + 公网 IP 保有费 ¥0.040 = **¥0.1107**。那 4 分钱不在 ECI 的价目里, 但每个实例
# 都自动创建一个 EIP。
# ⚠️ ECI 价格非线性: 1vCPU/2GiB 是 ¥0.2205 (3.12 倍, 不是 2 倍)。别按比例推。
# 在**积分也用满**的前提下, 各档的盈亏平衡机时:
#     Plus 月付 348h / 首月 228h / 年付 168h
#     Pro       838h 起
#     Max      1676h 起
# 原来 Plus 含 180h —— 月付尚有余量, 但首月与年付的含量已经**越过平衡点**,
# 重度用户是净亏。Pro/Max 余量以千小时计, 不受影响。
# 所以只降 Plus: 180h -> 100h (每天 3.3 小时, 对 $10 档合理)。
# 按 0.5vCPU/1GiB 的实价, Plus 年付余量 68 小时、满用毛利 15%, 三档全部健康。
# ⚠️ 但 2026-08-20 起机时按**容器运行时长**计, 不再只算智能体干活的分钟 ——
# 同样 100 小时, 用户消耗得比以前快得多 (100h = 每天 3.3 小时)。若投诉集中在
# "机时不够用", 要动的是这个数, 不是计量口径。
TIERS = [
