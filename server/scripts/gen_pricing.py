...
* **Yearly is 12 months at a discount.** The list price struck through on the
  ...
  yearly and per-month representations therefore cannot diverge.
...
# Rates are product configuration rather than live FX quotes. Values are chosen
# so displayed tiers remain stable and consistently rounded.
RATES = {"USD": 1.0, "CNY": 7.0, "EUR": 0.90, "GBP": 0.78, "HKD": 7.8, "JPY": 150.0}
# Quantisation step, in whole units of the currency.
STEP = {"JPY": 100}

# The master table is expressed in whole US dollars. Workspace minutes represent
# running container time and remain a separate allowance from model credits.
TIERS = [
...
# NOT expire monthly.
#
# A pack must not grant more credits than the same-priced plan; tests enforce
# this relationship.
PACKS = [
    ("pack1000", 10, 1000, 0),
    ("pack5000", 50, 5000, 0),
...
COMMENT = (
    "由 server/scripts/gen_pricing.py 生成，请勿手改。基准 1 USD = 7 CNY = 100 积分；"
...
def effective(usd: float, off: int) -> Fraction:
    ...
    depending on which currency is displayed.
