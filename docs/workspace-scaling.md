...
⚠️ **上面这些是「实例状态变 Running」, 不是「用户能用」。** 后来用真实启动脚本
端到端跑了一次, 差别看得很清楚:

```
Running @ 18s          <- ECI 说容器起来了
socat: connect(127.0.0.1:3080) Connection refused   x3
dsh 应答 @ 25s  HTTP 200 / 14549 字节               <- 用户真正能用
```

**到可用是 ~25 秒**, 那 7 秒是 dsh 自己绑端口 (本机同镜像只要 2s —— economy
算力是突发型 CPU, 慢在这里)。判据关心的是 25 这个数, 仍在 30s 以内, 但余量比
19s 显得小得多。单次探测失败不是故障: 生产的 `_ready()` 本来就是轮询的。
...
## 五·五、单价与成本模型（2026-08-20 控制台实价，新加坡）

推了好几遍才凑齐, 记在这里免得下次重推。**都是控制台标价, 不是账单** —— 第一张
账单出来后要按实际用量校准。

| 项 | 单价 | 来源 |
|---|---|---|
| ECI 0.5 vCPU / 1 GiB 经济型 | ¥0.00001963/秒 = **¥0.0707/小时** | 创建页实测 |
| ECI 1 vCPU / 2 GiB 经济型 | **¥0.2205/小时** | 创建页实测 (¥0.00006126/秒) |
| 公网 IP 保有费 | **¥0.040/小时** | EIP 购买页 + NAT 购买页, 两处一致 |
| 公网流量费 | 阶梯计价 | ⚠️ 未计入下面的模型 |
| NAT 网关实例费 | ¥0.255/小时 = **¥186/月** | NAT 购买页 |
| NAT 网关 CU 费 | ¥0.255/CU | 按连接数/流量, 未计入 |

**ECI 的价格不是线性的**: 0.5vCPU/1GiB ¥0.0707 → 1vCPU/2GiB ¥0.2205 是
**3.12 倍**, 不是 2 倍。曾按线性推算, 低估 56%; 加上 EIP 后整体低估 44%。
凡是没在创建页上真看过的规格, 都不要按比例推。

EIP 那 4 分钱也容易漏 —— 它不在 ECI 的价目里, 但每个实例都自动创建一个。

| 规格 | 算力 | +EIP | 每小时 |
|---|---|---|---|
| **0.5 vCPU / 1 GiB (生产在用)** | ¥0.0707 | ¥0.040 | **¥0.1107** |
| 1 vCPU / 2 GiB | ¥0.2205 | ¥0.040 | ¥0.2605 |

### 为什么选 0.5 vCPU

"半个 CPU"不是半颗核心, 是调度份额: 容器里 `nproc` 仍是 1, cgroup 也没有硬配额
(`cpu.max` = `-1/100000`), 只是分到的 CPU 时间少一半。实测:

| | 0.5 vCPU / 1 GiB | 1 vCPU / 2 GiB |
|---|---|---|
| 单线程 5 秒算力 | 43,320 | 75,317 (1.74×) |
| 双线程合计 | 42,745 | 68,246 |
| `npm install express` | 8s | 5s |
| 到 Running | 18s | 21s |
| 每小时成本 | **¥0.1107** | ¥0.2605 (2.35×) |

多花 2.35 倍的钱买 1.74 倍的算力, 而这个负载绝大多数时间在等 LLM、等网络、等
NFS。注意**双线程并不比单线程快** —— 两种规格 `nproc` 都是 1, 多开线程只是在同
一份配额里互抢, 所以 1 vCPU 也买不到"并行"。
真正的风险是内存而不是 CPU: 1 GiB 装大型原生依赖可能 OOM, 但那会在事件里明确
报出来, 届时单独调大比让所有人先付 2.35 倍稳妥。

### NAT 还是按实例 EIP

```
交叉点 = NAT 固定月费 ÷ EIP 每小时价
       = (186 + 29) ÷ 0.04 = 5384 实例小时/月
       = 平均并发 7.4 个
```
(186 = NAT 实例费, 29 = NAT 自身那个 EIP 的保有费; **未含 CU 与流量**, 算上之后
交叉点还要更高。)

**平均并发低于 7.4 就用按实例 EIP。** 换 NAT 的真正理由不是省钱, 是两件事:
  · EIP 配额是**账号级**的天花板 (当前 20, 已申请 200), NAT 之后不再受它约束;
  · 实例可以完全没有公网 IP, 从互联网侧根本不可达。
在并发够不着天花板之前, 为固定月费换这两样不划算。

### 什么时候重算

第一张账单同时给出 ECI 与 EIP 两项, 那时才有真实的「平均并发 × 平均时长」。
届时要重算的有三处: 上表里两个标 ⚠️ 的推算值、套餐的盈亏平衡机时
(server/scripts/gen_pricing.py 的 TIERS 注释)、以及 NAT 交叉点。
