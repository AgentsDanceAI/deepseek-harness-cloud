...
`0.2.0` 提供 npm 和 Python 两套等价的版本锁定安装器。两者默认使用本地试用模式，
...
```bash
npx --yes @agentsdanceai/dsh-cloud@0.2.0 init --mode selfhost \
  --domain cloud.example.com --admin-email admin@example.com
$EDITOR dsh-cloud/.env  # 配置 SMTP 或 Google/GitHub OAuth，以及模型上游
npx --yes @agentsdanceai/dsh-cloud@0.2.0 doctor dsh-cloud
npx --yes @agentsdanceai/dsh-cloud@0.2.0 up dsh-cloud --wait
```
