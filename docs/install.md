...
Release `0.4.0` has two equivalent, version-locked installers. Both default to
...
Release `0.4.0` uses the exact registry coordinates
`@agentsdanceai/dsh-cloud@0.4.0` and `dsh-cloud==0.4.0`. Keep the explicit version
...
```bash
npx --yes @agentsdanceai/dsh-cloud@0.4.0 init --mode selfhost \
...
npx --yes @agentsdanceai/dsh-cloud@0.4.0 doctor dsh-cloud
npx --yes @agentsdanceai/dsh-cloud@0.4.0 up dsh-cloud --wait
```
